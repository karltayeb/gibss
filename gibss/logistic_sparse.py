...
from gibss.logistic import summarize_susie, SusieFit, SusieSummary
...
@partial(jax.tree_util.register_dataclass,
         data_fields=['b', 'llr', 'lbf', 'alpha', 'lbf_ser', 'pi', 'prior_variance', 'optstate'], meta_fields=[])
@dataclass
class SparseSER:
    b: ArrayLike
    llr: ArrayLike
...
def make_sparse_logistic_ser1d(X_sp, y, prior_variance=1.0):
    ...
    def update_b(state, psi0):
        ...
        state = SparseSER(optstate.x, optstate.f, lbf, alpha, lbf_ser, state.pi, state.prior_variance, optstate)
        return state

    def logistic_ser_1d(psi, fit):
        if fit is None:
            p, n = X_sp.shape
            opt0 = OptState(jnp.ones(p), -jnp.inf * jnp.ones(p), jnp.ones(p), jnp.ones(p), jnp.ones(p))
            state0 = SparseSER(jnp.zeros(p), 0, 0, 0, -jnp.inf, jnp.ones(p)/p,  prior_variance, opt0)
...
def fit_logistic_susie2(X, y, L=10, prior_variance=1., maxiter=50, tol=1e-3, **kwargs):
    X_sp = sparse.csr_matrix(X)
    y = ensure_dense_and_float(y)
    fit = fit_logistic_susie(X, y, L=L, prior_variance=prior_variance, maxiter=maxiter, tol=tol, **kwargs)
