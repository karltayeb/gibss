...
from gibss.additive import (
    fit_additive_model,
    make_fitfun,
    AdditiveModel,
    update_additive_model,
)
from gibss.logistic import UnivariateRegression, make_fixed_fitfun, SusieFit
...
def nloglik(coef, x, y, offset, prior_variance):
    ...
    ll = jnp.sum(y * psi - jnp.logaddexp(0, psi)) + norm.logpdf(
        coef[1], 0, jnp.sqrt(prior_variance)
    )
    return -ll
...
def compute_wakefield_lbf(betahat, s2, prior_variance):
    lbf = norm.logpdf(
        betahat, loc=0, scale=jnp.sqrt(s2 + prior_variance)
...
def wakefield(
    ...
    solver = newton_factory(
        Partial(nloglik_mle, x=x, y=y, offset=offset),
        maxiter=maxiter,
        tol=tol,
        alpha=alpha,
        gamma=gamma,
    ...
    return UnivariateRegression(
        lbf + nullfit.logp, lbf, beta, prior_variance, prior_variance, state
    )
...
def estimate_prior_variance_wakefield(fits, prior_variance_init):
    ...
    fopt = gd_factory(f, maxiter=100, init_ss=1.0)
    return jnp.exp(fopt(jnp.atleast_1d(jnp.log(prior_variance_init))).x)[0]


@partial(jax.vmap, in_axes=(0, None))
def update_prior_variance_wakefield(
    fit: UnivariateRegression, prior_variance: float
...
def compute_lbf_score(x, y, offset, nullfit, prior_variance):
    ...
    b = jnp.array([nullfit.beta, 0.0])
    grad = jax.grad(nloglik)(b, x, y, 0.0, prior_variance)
    hess = jax.hessian(nloglik)(b, x, y, 0.0, prior_variance)
...
@partial(jax.vmap, in_axes=(0, None))
def update_prior_variance_lapmle(
    fit: UnivariateRegression, prior_variance: float
) -> UnivariateRegression:
    # extract state
    betahat = fit.state.x[1]
...
def estimate_prior_variance_lapmle(
    fits: UnivariateRegression, prior_variance_init: float
) -> float:
    """Estimate prior variance using Wakefield approximation"""
...
def hermite_grid_factory(m):
    ...
    def hermite_grid(
        ...
        def hermite_inner(coef_init, prior_variance):
            solver = newton_factory(
                Partial(
                    nloglik, x=x, y=y, offset=offset, prior_variance=prior_variance
                ),
                maxiter=maxiter,
                tol=tol,
                alpha=alpha,
            ...
            logp = jax.scipy.special.logsumexp(
                nll - norm.logpdf(nodes, loc=mu, scale=sigma) + jnp.log(weights)
            )
            lbf = logp - nullfit.logp
...
def fit_null(y, offset, maxiter=50, tol=0.1, alpha=0.1, gamma=-0.1):
    ...
    solver = newton_factory(
        Partial(nloglik0, y=y, offset=offset),
        maxiter=maxiter,
        tol=tol,
        alpha=alpha,
        gamma=gamma,
    ...
    params = state.x
    return UnivariateRegression(-state.f, 0, state.x[0], 0.0, state)
...
def logistic_ser_wakefield(
    coef_init, X, y, offset, prior_variance, maxiter=50, tol=0.1, alpha=0.1, gamma=0.0
):
    vwakefield = jax.vmap(
        Partial(
            wakefield,
...
def logisticprofile_hermite_ser_init(X, y, psi, fit=None, warm=False):
    """
...
def make_logisticprofile_hermite_ser(
    X,
    prior_variance=1.0,
    m=1,
    newtonkwargs={"maxiter": 50, "tol": 1e-5, "alpha": 0.1, "gamma": 0.0},
):
    def logisticprofile_hermite_ser(psi, fit, y):
        coef_init = logisticprofile_hermite_ser_init(X, y, psi, fit=fit, warm=True)
        fit = logistic_ser_hermite(
            coef_init, X, y, psi, prior_variance, m, newtonkwargs
        )

        return fit

    return logisticprofile_hermite_ser


def fit_logisticprofile_hermite_susie(
    X, y, L, prior_variance=1.0, m=1, maxiter=100, tol=1e-5
):
    fitfun = Partial(make_logisticprofile_hermite_ser(X, prior_variance, m), y=y)
...
def fit_logistic_susie(
    X, y, L=5, method="hermite", warm=True, serkwargs: dict = dict(), **kwargs
):
    match method:
        case "hermite":
            ...
            fitfun = make_fitfun(
                ...
                Partial(logisticprofile_hermite_ser_init, warm=warm),
                defaultserkwargs,
            )
        case "hermite_eb":
        ...
        case "hermite_eb":
            defaultserkwargs = dict(
                prior_variance_grid=jnp.array(
                    [1e-6, 1e-4, 1e-2, 1e-1, 0.5, 1.0, 2.0, 4.0]
                ),
            ...
            fitfun = make_fitfun(
                ...
                Partial(logisticprofile_hermite_ser_init, warm=False),
                defaultserkwargs,
            )
            if warm:
                warnings.warn(
